...
七组测试覆盖恢复时序、Windows 会话保护、本机与虚拟键鼠识别、回放控制和白名单查询、菜单语言及坐标、补丁引擎，以及集成生命周期。Hook 测试只使用临时内存或测试函数，不修改 NVIDIA 进程、自启设置或 Windows 会话，也不发送键盘输入。实际录制效果与驱动兼容性仍需在使用的 NVIDIA 硬件上验证。
