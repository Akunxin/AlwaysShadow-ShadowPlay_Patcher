...
- 可选的 **RDP 回放 0 秒修复**：返回物理机后自动关闭再开启 NVIDIA 信息浮窗，处理回放显示开启但无法保存的异常；默认关闭。
...
如果 RDP 断开后，回到物理机发现即时重放显示“保存最后 0 秒”、无法保存，并且必须关闭再开启 NVIDIA 信息浮窗才能恢复，可在托盘菜单勾选 **RDP 回放 0 秒修复（重启信息浮窗）**。此功能默认关闭，选择保存在当前用户的 `HKCU\Software\AlwaysShadow\RdpOverlayRecovery` 中，跨重启和升级保留。

开启开关会安排一次修复当前会话；之后每次检测到 RDP 连接或断开，也只安排一次。回到同一账户的本地桌面，操作物理键鼠并等待约十秒后，程序暂停补丁，通过 NVIDIA App 的原生接口关闭再开启信息浮窗，然后再等待约十秒、重新加载控制信息并按现有规则恢复即时重放。即使“回放已开启”的状态值没有变化，也会执行这次修复；程序并非通过读取缓冲时长来判断是否卡住。

修复遵守暂停、白名单、限定游戏和本机输入规则。单纯锁屏、解锁、显示器变化或重新加载设置不会触发新的浮窗重启；取消勾选会取消尚未开始的修复。已经在 NVIDIA 设置中关闭的浮窗会被跳过。浮窗重启会中断正在进行的录制，并清空未保存的回放缓冲，与手动切换该设置的影响相同。

此功能使用未公开的 NVIDIA 原生接口，已在 NVIDIA App 11.0.9.251 上验证接口加载和只读查询。接口不可用时会记录日志并跳过，不会反复重启；如果开启失败，最多再尝试开启一次，仍失败时会提示手动恢复。可通过**打开日志文件夹**查看 `RDP replay repair` 记录，或关闭再开启本开关以安排重试。实际 RDP 切换后请确认回放秒数增长、能够保存并播放视频；[实现与验证说明](docs/nvidia-overlay-api.md) 记录了接口边界和测试范围。
...
十一组测试覆盖恢复时序、Windows 会话保护、本机与虚拟键鼠识别、回放控制和白名单查询、RDP 浮窗修复及失败恢复、菜单语言及坐标、托盘启动重试与 Explorer 重建、Release 更新检查与数字版本比较、配置保存与迁移、补丁引擎，以及集成生命周期。设置与浮窗测试使用模拟注册表和 NVIDIA 接口，Hook 测试只使用临时内存或测试函数；不会访问实际用户设置、修改 NVIDIA 进程、自启设置或 Windows 会话，也不发送键盘输入。实际录制效果与驱动兼容性仍需在使用的 NVIDIA 硬件上验证。
...
| `HKCU\Software\AlwaysShadow` | 补丁配置、浏览器与保护开关、RDP 浮窗修复开关、更新偏好 |
