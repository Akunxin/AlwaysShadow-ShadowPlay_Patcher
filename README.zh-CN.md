...
- 自动恢复即时回放，支持远控结束、回到本地桌面或解锁后的重试。
- 集成 2.0 补丁引擎：导出函数 Hook、特征码扫描、外部 JSON 配置、逐项状态、NVIDIA 目标进程重启后自动重打补丁，以及补丁还原。
...
1. 按下文说明编译当前源码，或在项目发布页面获取包含集成补丁引擎的程序包。
2. 将 `AlwaysShadow.exe` 和 `patches.json` 放在同一个可写文件夹中。配置文件缺失时，程序会尝试创建默认配置。
...
默认 [patches.json](patches.json) 包含：
...
如果只需要自动恢复，可取消勾选**补丁保护**。需要尝试浏览器补丁时，单独勾选**浏览器检测补丁（实验性）**；此选择会保存到程序旁的 `patches.json`。

### 修改配置

编辑**可执行文件旁边**的配置，然后选择**重新加载设置**。本地编译时应编辑 `bin/patches.json`；修改源码根目录的副本不会覆盖已经存在的运行配置。
...
重新加载时，先还原当前补丁，再校验新配置。配置无效或还原失败时，会在**查看补丁状态…**和日志中报告；引擎保留还原记录并继续重试。
...
使用微软 RDP 时，AlwaysShadow 会在会话远程连接、断开或锁屏期间暂停。回到物理机，并用**同一个 Windows 账户**登录或解锁后，会等待约十秒让 NVIDIA 初始化，再重新加载控制信息并重试。首次尝试通常发生在本地桌面恢复可用后的约 10–20 秒。

向日葵等部分远控软件不发送 Windows 会话通知，因此程序还会持续检查即时回放状态。连续两次尝试失败后，重试间隔约为三十秒；远控结束、NVIDIA 重新允许录制后，后续尝试即可恢复。无需退出常驻的远控服务。
...
生成文件为 `bin/AlwaysShadow.exe`，运行配置为 `bin/patches.json`。C 代码使用 GCC，补丁引擎使用 G++ / C++20，最终采用静态链接。`tags=local` 可避免构建元数据依赖 GitHub CLI 或网络。重新编译会保留已有的运行配置。

六组测试覆盖恢复时序、Windows 会话保护、回放控制和白名单查询、菜单语言及坐标、补丁引擎，以及集成生命周期。Hook 测试只使用临时内存或测试函数，不修改 NVIDIA 进程、自启设置或 Windows 会话，也不发送键盘输入。实际录制效果与驱动兼容性仍需在使用的 NVIDIA 硬件上验证。
